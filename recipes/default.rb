...
include_recipe %w{php php::module_mysql php::module_gd php::module_apc php::module_memcache}
...
#  command "git clone --recursive --branch #{node['drupal']['branch']} #{node['drupal']['repo']} #{File.dirname(node['drupal']['dir'])}/drupal && mv #{File.dirname(node['drupal']['dir'])}/drupal #{File.basename(node['drupal']['dir'])} && \
